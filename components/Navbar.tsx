...
import { Leaf, Globe, Sun, Moon, Type, BookOpen, ShoppingBag, TrendingUp, Scan, Plane, User as UserIcon, LogOut, History, Receipt, FileText } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  onNavigateToSchemes, onNavigateToExpenses, onOpenLogin
}) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 border-b no-print ${
      isScrolled 
      ...
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <div className="flex lg:hidden items-center gap-2">
            <button onClick={onNavigateToAgriDrone} className="p-2 rounded-full bg-gold/10 text-gold">
              <Plane size={18} />
            </button>
            <button onClick={onNavigateToDiseaseDetect} className="p-2 rounded-full bg-emerald-500/10 text-emerald-500">
              <Scan size={18} />
            </button>
            <button onClick={onNavigateToMarket} className="p-2 rounded-full bg-black/5 dark:bg-white/5 text-gold">
              <TrendingUp size={18} />
            </button>
            <button onClick={onNavigateToCropGuide} className="p-2 rounded-full bg-black/5 dark:bg-white/5 text-emerald-600 dark:text-emerald-400">
              <BookOpen size={18} />
            </button>
            <button onClick={onNavigateToShop} className="p-2 rounded-full bg-black/5 dark:bg-white/5 text-gray-500">
              <ShoppingBag size={18} />
            </button>
            <button onClick={onNavigateToSchemes} className="p-2 rounded-full bg-blue-500/10 text-blue-500">
              <FileText size={18} />
            </button>
            <button onClick={onNavigateToExpenses} className="p-2 rounded-full bg-gold/10 text-gold">
              <Receipt size={18} />
            </button>
          </div>
          ...
          <div className="relative ml-2">
            {user ? (
              <>
                ...
                >
                  <div className="w-6 h-6 rounded-full bg-gold flex items-center justify-center text-[10px] font-bold text-black">
