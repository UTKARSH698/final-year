...
export const MarketAnalysis: React.FC<MarketAnalysisProps> = ({ onBack }) => {
  ...
  const handleSetAlert = () => {
    ...
    alert(`Price alert set for ${selectedCrop}!`);
  };
    ...
    <div className="min-h-screen pt-24 pb-20 px-6 bg-ivory dark:bg-obsidian">
      <div className="max-w-6xl mx-auto p-4 rounded-[3rem]" id="market-report-content">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12 no-print">
          ...
          <select value={selectedCrop} onChange={(e) => { setSelectedCrop(e.target.value); fetchForecast(e.target.value); }} className="bg-black/5 dark:bg-white/5 px-6 py-3 rounded-xl text-sm font-bold">
            {MANDI_RATES.slice(0, 10).map(m => <option key={m.crop} value={m.crop}>{m.crop}</option>)}
          </select>
        </div>

        {loading ? (
          <div className="h-[60vh] flex items-center justify-center"><Loader2 className="animate-spin text-gold" size={48} /></div>
        ) : forecast && (
                ...
                <div className="bg-white dark:bg-charcoal border border-black/5 dark:border-white/10 rounded-[3rem] p-10 shadow-2xl relative overflow-hidden">
                   <div className="flex justify-between items-start mb-12">
                      ...
                      <div className="flex gap-3 no-print">
                         <button onClick={handleExportPDF} disabled={isExporting} className="p-3 rounded-xl bg-gray-100 dark:bg-white/5 text-gray-500 hover:text-gold transition-colors">
                            {isExporting ? <Loader2 size={18} className="animate-spin" /> : <Download size={18} />}
                         </button>
                         <button onClick={handleShare} className="p-3 rounded-xl bg-gray-100 dark:bg-white/5 text-gray-500 hover:text-gold transition-colors"><Share2 size={18} /></button>
                         <button onClick={handleWhatsApp} className="p-3 rounded-xl bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500/20 transition-colors" title="Share on WhatsApp">
