...
import { CropRotationPlan, CropRotationStep, Language } from '../types';
...
const StepCard: React.FC<{ step: CropRotationStep; index: number; total: number; language?: string }> = ({ step, index, total, language = 'English' }) => {
            ...
            {expanded && (
              ...
              >
                <div className="mt-4 space-y-3 pt-4 border-t border-black/5 dark:border-white/5">
                  <div>
                    <span className="text-[9px] font-jakarta font-bold text-gold uppercase tracking-widest">{t('whyThisCrop')}</span>
                    <p className="text-sm text-gray-700 dark:text-gray-300 font-medium mt-1">{step.reason}</p>
                  </div>
                  <div className="p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/10">
                    <span className="text-[9px] font-jakarta font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">{t('soilBenefit')}</span>
        ...
        <div className="sticky top-0 z-10 bg-ivory/95 dark:bg-obsidian/95 backdrop-blur-xl px-8 py-6 border-b border-black/5 dark:border-white/5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-outfit font-bold text-gray-900 dark:text-white">{t('smartRotation')}</h2>
        ...
        <div className="px-8 py-6">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              <p className="text-sm font-inter text-gray-500 dark:text-gray-400">{t('analysingCycles')}</p>
            </div>
          ) : error ? (
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-3 p-4 rounded-2xl bg-gold/5 border border-gold/20">
                ...
                <div className="flex-1">
                  <p className="text-[9px] font-jakarta font-bold text-gold uppercase tracking-widest">{t('currentHarvest')}</p>
              ...
              <div className="space-y-4">
                {plan.steps.map((step, i) => (
                  <StepCard key={i} step={step} index={i} total={plan.steps.length} language={language} />
                ))}
              </div>
